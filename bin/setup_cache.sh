...
test -d "$IVY_CACHE" || \
  ...
  echo "Ivy cache has been set up for $IVY_CACHE.")
...
test -d "$COURSIER_CACHE" || \
  ...
  echo "Coursier cache context has been set up for $COURSIER_CACHE.")
...
test -d "$SBT_HOME_DIR" || \
  ...
  echo "Sbt cache context has been set up for $SBT_HOME_DIR.")
